...
&& [ $ENABLE_SNMPv3 == "1" ] && {
    ...
    && sed -E -i 's/^(rocommunity\s+public\s+default\s+-V\s+systemonly)/#\1/g' /etc/snmp/snmpd.conf \
    && sed -E -i "s/^rouser\s+authOnlyUser\s*$/rouser ${snmpUser}/g" /etc/snmp/snmpd.conf \
    ...
    && sed -E -i 's/^(trapsink\s+localhost\s+public)$/#\1/g' /etc/snmp/snmpd.conf \
    && sed -E -i "s/^iquerySecName\\s+internalUser/iquerySecName ${snmpUser}/g" /etc/snmp/snmpd.conf \
    && sed -E -i "s/^rouser\\s+internalUser/rouser ${snmpUser}/g" /etc/snmp/snmpd.conf
